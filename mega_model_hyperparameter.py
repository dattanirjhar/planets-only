...
OUTPUT_DIR = 'datasets'
OUTPUT_FILE = 'mega_unified_dataset_v3.csv'
...
def feature_engineering(df):
    ...
    if 'pl_rade' in df.columns and 'st_rad' in df.columns:
        ...
        print("⚠️ Skipped feature 'pl_rad_to_star_rad_ratio': Missing one or more required columns ('pl_rade', 'st_rad').")
    ...
    if 'st_mass' in df.columns and 'st_rad' in df.columns:
        ...
        print("⚠️ Skipped feature 'st_density': Missing one or more required columns ('st_mass', 'st_rad').")
    ...
    if 'pl_trandep' in df.columns and 'pl_trandeperr1' in df.columns:
        ...
        print("⚠️ Skipped feature 'pl_signal_significance': Missing one or more required columns ('pl_trandep', 'pl_trandeperr1').")
...
def main():
    ...
    try:
        ...
        # --- NEW STEP: Hyperparameter Tuning with RandomizedSearchCV ---
        ...
        # n_iter controls how many different combinations to try. cv is for cross-validation.
        random_search = RandomizedSearchCV(estimator=xgb, param_distributions=param_dist, 
                                           n_iter=25, cv=3, verbose=1, n_jobs=-1, random_state=42)
        ...
        # Use the best found model for final evaluation
        best_model = random_search.best_estimator_
        print("\n--- Training final optimized model with best parameters... ---")
        
        print("\n--- Performing the Ultimate 3-Way Segmented Validation on Tuned Model ---")
